...
static FACTORY_VTABLE: IPluginFactory = {
    ...
    extern "system" fn get_factory_info(this: *mut c_void, info: *mut PFactoryInfo) -> TResult {
        let info = unsafe { &mut *info };

        let vendor = b"vendor\0";
        unsafe { std::ptr::copy(vendor.as_ptr() as *const c_char, info.vendor.as_mut_ptr(), vendor.len()) };
        ...
        unsafe { std::ptr::copy(email.as_ptr() as *const c_char, info.email.as_mut_ptr(), email.len()) };
    ...
    extern "system" fn count_classes(this: *mut c_void) -> i32 {
        1
    }
    ...
    extern "system" fn get_class_info(
        ...
        unsafe { std::ptr::copy(category.as_ptr() as *const c_char, info.category.as_mut_ptr(), category.len()) };

        let name = b"vst3 test\0";
        unsafe { std::ptr::copy(name.as_ptr() as *const c_char, info.name.as_mut_ptr(), name.len()) };
